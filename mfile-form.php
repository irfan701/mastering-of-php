...
if (isset($_FILES['photo'])) {
    ...
    for ($i = 0; $i < $totalFiles; $i++) {
        ...
        $destination = 'files/' . $_FILES['photo']['name'][$i];
        $fileType = $_FILES['photo']['type'][$i];
        $allowedTypes = ['image/png', 'image/jpg', 'image/jpeg'];
        $fileSize = $_FILES['photo']['size'][$i] < 5 * 1024 * 1024;

        if (in_array($fileType, $allowedTypes) != false && $fileSize) {
